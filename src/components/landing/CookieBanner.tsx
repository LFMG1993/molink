                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-start gap-3">
                        ...
                        <p className="text-sm text-white/80">
                            ...
                            <Link to="/cookie-policy" className="font-semibold text-accent hover:underline">{t('landing.cookie_banner.policy_link')}</Link>.
                        </p>
                    </div>
                    <div className="flex-shrink-0 flex items-center gap-3 w-full sm:w-auto">
                        <button onClick={handleReject}
                                className="w-full sm:w-auto px-4 py-2 rounded-md border border-white/30 hover:bg-white/10 transition-colors">{t('landing.cookie_banner.reject')}</button>
                        <button onClick={handleAccept}
                                className="w-full sm:w-auto px-4 py-2 rounded-md bg-accent hover:bg-red-800 transition-colors font-semibold">{t('landing.cookie_banner.accept')}</button>
                    </div>
                </div>
            </div>
        </div>
